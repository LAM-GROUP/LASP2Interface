...
# Begin simulation
lammpsRun = Popen('mpirun --map-by core --bind-to core python3.7 /tmpdir/fresseco/install/LASP2Interface/interfaceLAMMPS.py --start -config '+inputFile+' -iteration '+str(trainings)+' > lasp2_'+str(trainings)+'.out', shell=True, stderr=subprocess.PIPE, env=os.environ)
lammpsRun.wait()
...
while True:
    if re.match('^50', exitErr): #Exit code returned when the flag for training is activated
        ...
        trainings += 1
        lammpsRun = Popen('mpirun --map-by core --bind-to core python3.7 /tmpdir/fresseco/install/LASP2Interface/interfaceLAMMPS.py --restart -config '+inputFile+' -iteration '+str(trainings)+' > lasp2_'+str(trainings)+'.out', shell=True, stderr=subprocess.PIPE, env=os.environ)
        lammpsRun.wait()
        exitErr = lammpsRun.stderr.read().decode()
    else:
        break
